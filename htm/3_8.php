...
<body  class= "overflow-hidden example-box dark" id="custom">
  <!-- Верхняя панель -->
  <nav class = "navbar navbar-expand-md navbar-dark bg-dark text-left" >
    <div class="container-fluid ">
      ...
      <a class="navbar-brand" href="../../0_0_1catalog.php">
        ...
        <ul class="navbar-nav mr-auto">
          <li class="nav-item dropdown"><a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> Мужская обувь </a> 
            <div class="dropdown-menu" aria-labelledby="navbarDropdown">              
              <a class="dropdown-item" href="http://http://localhost/htm/1_5.php"> Кеды </a>
...
<?php 
  ...
  $shoes_all = $array1[4]; ?> 
	 ...
	 <?php $k =0;
	  $count_in_row = 3; 
	  for ($y = 0; $y < (count($array1[0])/$count_in_row); $y++){?>
	  <div class="card-deck"> 
	  <?php for($z = 0; $z < $count_in_row; $z++){
	  if ($k == count($names)){ 
	  ?>
	  <?php break;}
	  ?>
        <div class="card">
          <img class="card-img-top" src="<?php echo $img[$k]?>" alt="Card image cap">
          <div class="card-body">
            <h1 class="card-title"><?php echo $pricef1[$k] ?> Р</h1>
            <h3 class="card-title"><?php echo $names[$k] ?></h3>
            <p class="card-text"><?php echo $shoes[$k] ?></p>
            <p class="card-text"><small class="text-muted"><?php echo $pricefall[$y] ?> Р за пак из <?php echo $shoes_all[$y] ?>пар</small></p>
          </div>
		  </div>
	  <?php $k++; }
			  if ($k == count($names)){ 
	  ...
	  <br/>
	  <?php }
	  if(count($array1[0])%$count_in_row != 0){
		for ($y = 0; $y < (count($array1[0])%$count_in_row); $y++){
		   ?><div class="card bg-transparent hell">
          <div class="card-body special-card">
            <h1 class="card-title special-card"></h1>
            <h3 class="card-title special-card"></h3>
            <p class="card-text special-card"></p>
            <p class="card-text special-card"><small class="text-muted special-card"></small></p>
          </div>
		  </div>
		  <?php
		}
